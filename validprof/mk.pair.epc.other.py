...
for DTime in lDTime:
    ...
    for gprofPath in lgprofPath: 
        ...
        for var in lvar:
            ...
            varDir = tankbaseDir + '/utsumi/PMM/MATCH.GMI.V05A/%s/%04d/%02d/%02d'%(var, Year,Mon,Day)
    
            varPath= varDir + '/%s.%06d.npy'%(varName, oid)
    
            if not os.path.exists(xPath):
                continue    

            a2var = np.load(varPath)
            if angle == 'ABp083-137':
                pass
            elif angle == 'ABp000-220':
                a2var = a2var[:,83:137+1]
            else:
