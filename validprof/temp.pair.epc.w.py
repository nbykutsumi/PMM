...
#lvar = [['DPRGMI','NS/Input/surfaceElevation'],['DPRGMI','NS/Input/zeroDegAltitude']]
    ...
    for pmwPath in lpmwPath: 
        ...
        for [prod,varName] in lvar:
            ...
            if (varName in ['NS/PRE/heightStormTop']):
                a2stopd = ma.masked_greater(avarorg,32767).filled(32767).astype('int16')  # miss is converted from -9999.9 --> -9999
                #a2stopd = ma.masked_less(a2stopd,0)
                a2stopd = ma.masked_less_equal(a2stopd,0) # mask zero and less
                davar[prod,varName] = ave_9grids_2d(a2stopd, a1y, a1x, miss=-9999).filled(-9999).astype('int16')
            ...
            print prod, varName, davar[prod,varName].shape
        sys.exit()
        #--------------------------------- 
        for [prod,varName] in lvar:
            if varName == 'NS/PRE/heightStormTop':
                outvarName = 'stoprad'
