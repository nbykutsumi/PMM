...
def ave_9grids_2d(a2in, a1y, a1x, miss):
    ...
    print a
    print a2datTmp
    print 'nmask=',ma.masked_equal(a2datTmp,miss).mask.sum()
    ...
    for pmwPath in lpmwPath: 
        ...
        for [prod,varName] in lvar:
            ...
            if (prod=='DPRGMI')and(varName in ['NS/Input/zeroDegAltitude'
                ...
                avar = ma.masked_less(avar,0) # mask zero and less
                print 'No bef',avar
                print 'No bef',avar.mean()

                avar = ave_9grids_2d(avar, a1y, a1x, miss=-9999).filled(-9999).astype('int16')
                print 'No aft',ma.masked_less(avar,0).mean()


                a=np.arange(3*3).reshape(3,3)
                b=ma.masked_greater(a,3)
                print b
                print a.mean(), b.mean()
                print ave_9grids_2d(b, np.array([1]), np.array([1]), miss=-9999)
                sys.exit()
