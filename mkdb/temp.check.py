        ...
        for epcid_range in lepcid_range:
            ...
            for (obtnum, Year,Mon,Day,time0, time1) in lobt:   
                ...
                if   varName in ['zFactorCorrected','zFactorMeasured']:
                    ...
                    a1dprmask = np.array([False])
                    nydpr,nxdpr,nzdpr= Dat0.shape
                    a1yTmp  = ma.masked_outside(a1dpry, 0,nydpr-1)
                    a1xTmp  = ma.masked_outside(a1dprx, 0,nxdpr-1)
                    a1dprmask= a1dprmask + a1yTmp.mask + a1xTmp.mask
 
                    atmp= Dat0[a1yTmp.filled(0),a1xTmp.filled(0),:]
                    atmp[a1dprmask,:] = -9999.9


                    a1bin = np.arange(60)[::-1]
                    abin  = np.array([a1bin for i in range(atmp.shape[0])])

                    aminave= ma.masked_where(Dat<0, abin).min(axis=1)
                    aminorg= ma.masked_where(atmp<0, abin).min(axis=1)

                    for i in range(aminave.shape[0]):
                        
                        print i, aminave[i], aminorg[i]
                        #if aminave[i]>aminorg[i]:
                        if aminave[i]<aminorg[i]:
                            sys.exit()
