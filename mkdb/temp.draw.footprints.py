...
for itmp,y in enumerate([1000]):
    ...
    a2latdpr = Lat1[ay.min()-2:ay.max()+2+1, ax.min()-10:ax.max()+10+1]
    a2londpr = Lon1[ay.min()-2:ay.max()+2+1, ax.min()-10:ax.max()+10+1]
    ...
    lllat = a2latdpr.min()+0.35
    urlat = a2latdpr.max()-0.5
    lllon = a2londpr.min()-0.1+0.8
    urlon = a2londpr.max()+0.1-0.8
    ...
    plt.title( stitle )
    figPath = '/home/utsumi/temp/temp.GMI.DPR.centers.%d.png'%(y)
    plt.savefig(figPath)
    print figPath
