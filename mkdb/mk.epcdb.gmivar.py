...
lepcid_range = [[0,2500],[2500,5000],[5000,7500],[7500,10000]]
...
#lvar = [['gmi','S1/Latitude'],['gmi','S1/Longitude'],['gprof','S1/surfaceTypeIndex']]
#lvar = [['gmi','S1/Tc']]
...
#lvar = [['gmi','S1/pYXpmw']]
lvar = [['gmi','S1/gNum']]
#lvar = [['gprof','S1/ScanTime/Year']]
#lvar = [['gprof','S1/ScanTime/mdhms']]
#lvar = [['gmi','S1/LatLon']]
...
,'S1/surfaceTypeIndex':'int8'
,'S1/surfacePrecipitation':'int8'
,'S1/pYXpmw': 'int8'
,'S1/gNum': 'int16'
}
        ...
        for epcid_range in lepcid_range:
            ...
            for (obtnum, Year,Mon,Day,time0, time1) in lobt:   
                ...
                if varName  =='epc':
                    ...
                    a2epc   = np.load(epcPath).reshape(-1,NEM)
                    ...
                    X   = X[:,cx-w:cx+w+1]
                    Y   = Y[:,cx-w:cx+w+1]
                    Dat = zeros([nytmp,nxtmp,2],int8)
                    Dat[:,:,0] = Y
                    Dat[:,:,1] = X
                    Dat = Dat.reshape(-1,2).astype(int8)
