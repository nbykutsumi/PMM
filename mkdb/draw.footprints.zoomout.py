...
for itmp,y in enumerate([100,500,750,1000,1500,2000,2500]):
    ...
    ax   = X[iy:ey+1,27-2:27+2+1]
    ay   = Y[iy:ey+1,27-2:27+2+1]
    
    a2latgmi = Lat0[iy:ey+1,cx-10:cx+10+1]
    a2longmi = Lon0[iy:ey+1,cx-10:cx+10+1]
    
    a2latdpr = Lat1[ay.min()-2:ay.max()+2+1, ax.min()-2:ax.max()+2+1]
    a2londpr = Lon1[ay.min()-2:ay.max()+2+1, ax.min()-2:ax.max()+2+1]
    
    a2latdpr1= Lat1[ay,ax]
    a2londpr1= Lon1[ay,ax]
    ...
    #-- test -----------
    a2latgmi = Lat0[iy:ey+1,:]
    a2longmi = Lon0[iy:ey+1,:]

    a2latdpr = Lat1[ay.min()-2:ay.max()+2+1, :]
    a2londpr = Lon1[ay.min()-2:ay.max()+2+1, :]

    lllat = a2latgmi.min()-1
    urlat = a2latgmi.max()+1
    lllon = a2longmi.min()-0.1
    urlon = a2longmi.max()+0.1

    print 'lonGMI-center=',a2longmiC.mean() 
    print 'lonDPR-center=',a2londprC.mean()
    print 'dif=',a2longmiC.mean() - a2londprC.mean()
    print Lat0.shape
    print Lat1.shape
    #---------------
    ...
    M.scatter(a2londpr, a2latdpr, marker='o',edgecolor='gray',facecolor='none',s=100)
    #M.scatter(a2londpr1, a2latdpr1, marker='o',edgecolor='red',facecolor='none',s=100)
    M.scatter(a2longmi, a2latgmi, marker='o',edgecolor='k',facecolor='none',s=100)
    
    #M.scatter(a2londprC, a2latdprC, marker='o',color='gray',s=100, alpha=0.7)
    ...
    M.scatter(a2longmiC, a2latgmiC, marker='o',color='green', s=100, alpha=0.7)
    

    
    #meridians = arange(int(lllon)-3,int(urlon)+3,0.1)
    #parallels = arange(int(lllat)-3,int(urlat)+3,0.1)
    meridians = arange(int(lllon)-3,int(urlon)+3,0.5)
    parallels = arange(int(lllat)-3,int(urlat)+3,0.5)
    ...
    plt.title( stitle )
    figPath = '/home/utsumi/temp/GMI.DPR.sample.%d.png'%(y)
    plt.savefig(figPath)
    print figPath
