...
a2pc_range     = zeros([nytmp,nxtmp/2+1]) # 11*(25+1)
a2pc_range_min = zeros([nytmp,nxtmp/2])   # 11*25
a2pc_range_max = zeros([nytmp,nxtmp/2])   # 11*25
...
for DTime in lDTime:
    ...
    for srcPathGMI in lsrcPathGMI:
        ...
        datoutTmp = gmi.load_var_granule(srcPathGMI,'S1/Tc')
        a3tb_s1 = datoutTmp[:,cx-w:cx+w+1,:]

        datoutTmp = gmi.load_var_granule(srcPathGMI,'S2/Tc')
        a3tb_s2 = datoutTmp[:,cx-w:cx+w+1,:]
