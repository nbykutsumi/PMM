  ...
  for binMin in lbinMin:
    for lndsea in llndsea:
      ...
      for i,clid in enumerate([99,1,3,4]):
        ...
        figPath = os.path.join(figDir, "%sdom.Boxplot.rat.min%.1f.%s.%s.png"%(dommask, binMin, lndsea, dclShortName[clid]))
      ...
      oPath   = os.path.join(figDir, "join.%sdom.Boxplot.rat.min%.1f.%s.png"%(dommask, binMin, lndsea))
