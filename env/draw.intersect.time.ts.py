    ...
    for prtype in lprtype:
        for height in lheight: 
            for ts in lts:
                for surf in lsurf:
                    for dt in ldtenv:
                        dsum[var,prtype,height,ts,surf,dt] = 0.0
                        dnum[var,prtype,height,ts,surf,dt] = 0
                        dclm[var,prtype,height,ts,surf,dt] = 0
                        dstd[var,prtype,height,ts,surf,dt] = 0
...
for (Year,Mon) in lYM:
    ...
    for DTime in lDTime:
        ...
        for latPath in llatPath:
            ...
            a1ts   = np.load(pairbaseDir + '/%s/%04d/%02d/%02d/%s.00.0km.%06d.npy'%('skt',Year,Mon,Day,'skt',oid))   
            #****************
            # Flags
            #----------------
            d1flagp = {}
            ...
            for ts in lts:
                ...
                d1flagt[ts] = ma.masked_inside( a1ts, ts0, ts1).mask

                
            d1flagt['s']    = ma.masked_inside(np.abs(a1lat), 20, 30).mask
            d1flagt['m']    = ma.masked_greater(np.abs(a1lat), 35).mask
                        ...
                        for height in lheight:
                            for ts in lts:
                                for surf in lsurf:
                                    ...
                                    a1flag = a1flagp * a1flagh * a1flagt * a1flags * a1flage
                                    ...
                                    dsum[var,prtype,height,ts,surf,dt] = dsum[var,prtype,height,ts,surf,dt] + s
                                    dnum[var,prtype,height,ts,surf,dt] = dnum[var,prtype,height,ts,surf,dt] + n


                                    dclm[var,prtype,height,ts,surf,dt] = dclm[var,prtype,height,ts,surf,dt] + clm
                                    dstd[var,prtype,height,ts,surf,dt] = dstd[var,prtype,height,ts,surf,dt] + std
...
for var in lvar:
    for height in lheight:
        for surf in lsurf:
            ...
            for prtype in lprtype:
                for ts in lts:
                    a1s = np.array([dsum[var,prtype,height,ts,surf,dt] for dt in ldtenv])
                    a1n = np.array([dnum[var,prtype,height,ts,surf,dt] for dt in ldtenv])

                    a1clm=np.array([dclm[var,prtype,height,ts,surf,dt] for dt in ldtenv])
                    a1std=np.array([dstd[var,prtype,height,ts,surf,dt] for dt in ldtenv])

                    d1y[prtype,ts] = ma.masked_invalid(ma.masked_where(a1n==0, a1s) / a1n) * coef
           
                    d1n[prtype,ts] = a1n 
                    d1clm[prtype,ts] = ma.masked_where(a1n==0, a1clm)/a1n
                    d1std[prtype,ts] = ma.masked_where(a1n==0, a1std)/a1n

            #-- Figure variables ---
            fig = plt.figure(figsize = (6,3))
            ax  = fig.add_axes([0.1,0.1,0.8,0.8])
            
            ax.plot(a1x, d1y['conv','cold'], '-',  color='b', label='conv/<10')
            ax.plot(a1x, d1y['strat' ,'cold'], '--', color='b', label='strat/<10')
            ax.plot(a1x, d1y['conv','cool'], '-',  color='green', label='conv/10-20')
            ax.plot(a1x, d1y['strat' ,'cool'], '--', color='green', label='strat/10-20')
            ax.plot(a1x, d1y['conv','warm'], '-',  color='orange', label='conv/20-26')
            ax.plot(a1x, d1y['strat' ,'warm'], '--', color='orange', label='strat/20-26')
            ax.plot(a1x, d1y['conv','hot'], '-',  color='red', label='conv/26<')
            ax.plot(a1x, d1y['strat' ,'hot'], '--', color='red', label='strat/26<')
