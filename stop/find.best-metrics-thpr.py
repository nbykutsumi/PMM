...
lexpr = [
         #'glb.v03.minrec%d.maxrec%d'%(DB_MINREC,DB_MAXREC)
...
for season in lseason:
    ...
    #                    ssearch = dprDir + '/2A.GPM.Ku.*.%06d.V06A.HDF5'%(oid)
                        ...
                        #fig = plt.figure()
                        #a2rnr = ma.masked_less(a2rnr,0)
                        #a2ret = ma.masked_less(a2ret,0)
                        #plt.scatter(a2rnr, a2ret)
                        #plt.savefig('/home/utsumi/temp/stop/temp.png')
                        #**************************************
                        # set precip <prmin to zero
                        #--------------------------------------
                        a2ret = ma.masked_less(a2ret, prmin).filled(0.0)
                        a2obs = ma.masked_less(a2obs, prmin).filled(0.0) 
                        #--- Mask ---------------------
                        a2mask  = a2masksurf
                    ...
                    # Save
                    #-------------------------------
                    stamp =  '%s.%s.%s.th%.1fmm.%04d.%02d'%(expr, region, surftype, thpr, Year, Mon)
                ...
                #stampOut = '%s-thpr%.1fmm.%s.%s.thpr%.1fmm.%s'%(expr, region, surftype, thpr, season)
