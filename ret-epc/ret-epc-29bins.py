...
#listPath= listDir + '/overpass.%s.%04d.%02d.csv'%(sateName,Year,Mon)
#f=open(listPath,'r'); lines=f.readlines(); f.close()
#lobtinfo = []
#for line in lines:
#    lorbinfo.append(map(int,line))  # [Year,Mon,Day,oid,iy,ey]
#
#    #-- Tc Path -------
...
#rangePath = coefDir + '/PC_MIN_MAX_29_no_overlap.txt'
...
for i,idx_db in enumerate(lidxset):
    ...
    for (y,x) in zip(a1y,a1x):  # in idx_db loop
        ...
        a3top_zmMS[y,x,:] = db.get_var('z_ka', nrec=1, origin=topirecMS).flatten()[-NLEV_DPR:] 
        ...
        a3top_zmNS[y,x,:] = db.get_var('z_ku', nrec=1, origin=topirecNS).flatten()[-NLEV_DPR:] 
        ...
        #a1tmp0 = a2prprofNSSC[idxtopNS]
        #a1tmp1 =  db.get_var('precip_prof_NS', nrec=1, origin=topirecNS).flatten()

        #a1tmpzm=  db.get_var('z_ka', nrec=1, origin=topirecNS).flatten()[-22:] 
