...
for DTime in lDTime:
    ...
    for lgmiPath in llgmiPath[:1]:
        ...
        # Comparison
        #----------------------------
        with h5py.File(srcPathTmp) as h:
            a3tb1batch = h['/S1/Tc'][:]
            a3tb2batch = h['/S2/Tc'][:]
            a2latbatch = h['/S1/Latitude'][:]
            a2lonbatch = h['/S1/Longitude'][:]

        a2xbatch = np.load(s2xPathTmp)
        a2ybatch = np.load(s2yPathTmp)
        a2t2mbatch= np.load(t2mPathTmp)

        y0 = 0
        for gmiPath in lgmiPath:
            oid = int(gmiPath.split('.')[-3])
            gmiPath = glob.glob(gmibaseDir + '/%04d/%02d/%02d/1C.GPM.GMI.XCAL2016-C.*.%06d.????.HDF5'%(Year,Mon,Day,oid))[0]
            s2xPath = glob.glob(matchbaseDir + '/S1.ABp000-220.GMI.S2.IDX/%04d/%02d/%02d/Xpy.1.%06d.npy'%(Year,Mon,Day,oid))[0]
            s2yPath = glob.glob(matchbaseDir + '/S1.ABp000-220.GMI.S2.IDX/%04d/%02d/%02d/Ypy.1.%06d.npy'%(Year,Mon,Day,oid))[0]
            t2mPath = glob.glob(matchbaseDir + '/S1.ABp000-220.MERRA2.t2m/%04d/%02d/%02d/t2m.%06d.npy'%(Year,Mon,Day,oid))[0]

            #-- read original orbit-wise data --- 
            ...
            a2xorg = np.load(s2xPath)
            a2yorg = np.load(s2yPath) 
            a2t2morg=np.load(t2mPath)
            #-- slice from batch ----------------
            ny = a2latorg.shape[0]
            a3tb1tmp = a3tb1batch[y0:y0+ny]
            a3tb2tmp = a3tb2batch[y0:y0+ny]
            a2lattmp = a2latbatch[y0:y0+ny]
            a2lontmp = a2lonbatch[y0:y0+ny]
            a2xtmp   = a2xbatch  [y0:y0+ny]
            a2ytmp   = a2ybatch  [y0:y0+ny]
            a2t2mtmp = a2t2mbatch[y0:y0+ny]
            #print a3tb1org.mean(),a3tb1tmp.mean() 
            print ''
            print oid,y0,ny
            print 'batch shape',a3tb1batch.shape
            print a3tb1org.shape,a3tb1tmp.shape
            
            #print a3tb1org[100,100], a3tb1org.mean()
            #print a3tb1tmp[100,100], a3tb1tmp.mean()

            print a3tb2org[100,100], a3tb2org.mean()
            print a3tb2tmp[100,100], a3tb2tmp.mean()

            #print a2latorg[100,100], a2latorg.mean()
            #print a2lattmp[100,100], a2lattmp.mean()

            #print a2lonorg[100,100], a2lonorg.mean()
            #print a2lontmp[100,100], a2lontmp.mean()

            #print a2lonorg[100,100], a2lonorg.mean()
            #print a2lontmp[100,100], a2lontmp.mean()

            #print a2xorg[100,100], a2xorg.mean()
            #print a2xtmp[100,100], a2xtmp.mean()

            #print a2yorg[100,100], a2yorg.mean()
            #print a2ytmp[100,100], a2ytmp.mean()

            #print a2t2morg[100,100], a2t2morg.mean()
            #print a2t2mtmp[100,100], a2t2mtmp.mean()
