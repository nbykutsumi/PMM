...
lidx_db = [4150]
#lidx_db = [4991,4150]
#liirecTarget=[40,41,42,43,44,45]
liirecTarget=[43]
nsample = 1000
#nsample = 10
...
for idx_db in lidx_db:
    ...
    a2prwatprofNS= ma.masked_invalid(a2prwatprofNS)
    ...
    a2prwatprofNS= ma.masked_less(a2prwatprofNS,0)
    ...
    for iirec,irec in enumerate(lirec):
        ...
        #--- Save --------------
        a1z  = (np.arange(NLEV_PRECIP)*0.25).reshape(1,-1)
        a2out= np.concatenate([a1z, a1prwatprofNS[::-1].reshape(1,-1), a2prwatprofNS[:,::-1]],axis=0)

        a1wtTmp = np.concatenate([np.array([0,0]), a1wt.filled(0.0)], axis=0)/wtsum

        print a1wt/wtsum
        a2out = np.concatenate([a1wtTmp.reshape(-1,1), a2out],axis=1)
        print a2out.shape
        sout = util.array2csv(a2out)
        outPath='/home/utsumi/temp/ret/entries.%05d.irirec.%05d.csv'%(idx_db, iirec)
