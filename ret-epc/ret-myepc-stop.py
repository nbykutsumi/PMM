...
#**************************************************************
    ...
    type_stop   = dargv['type_stop']  # stop   
...
if type_stop not in ['', 'rng','wgt']:
    print 'check type_stop',type_stop
    sys.exit()
...
if (stopPath !='')and(type_stop=='wgt'):
    a1pc_std = np.concatenate([a1pc_std, np.array([STD_STORMTOP])])
...
#****************************************************
# Rain No-rain (RNR) screening with first guess precipitation
#----------------------------------------------------
print ''
print ''
...
print ''
print ''
print 'Ater',a2mask.sum()
print ''
print ''

#****************************************************
# Mask pixels with invalid inputs
#----------------------------------------------------
if stopPath !='':
    a2mask = a2mask + ma.masked_less(a2stop, 0).mask
    a2mask = a2mask + ma.masked_invalid(a2stop, 0).mask
...
#a3prwatprofNS= ones([nyout,nxout,NLEV_PRECIP],float32)*miss  # stop
...
if flag_top_var == 1:
    ...
    #a3top_prwatprofNS = ones([nyout,nxout,NLEV_PRECIP],float32)*miss  # stop
...
for i,idx_db in enumerate(lidxset):
    ...
    for iidx_db, idx_db_expand in enumerate(lidx_db_expand):
        ...
        #a2prwatprofNSTmp = ma.masked_invalid(db.get_var('precip_water_prof_NS')[:,-NLEV_PRECIP:]).filled(-9999.)  # stop
        ...
        if stopPath !='':
            a1stopdbTmp= db.get_var('storm_height_ku')
        ...
        if (iidx_db==0):
            ...
            #a2prwatprofNS = a2prwatprofNSTmp   # stop
            ...
            #a2prwatprofNS = concatenate([a2prwatprofNS, a2prwatprofNSTmp]) # stop
            ...
            if stopPath !='':
                a1stopdb= concatenate([a1stopdb, a1stopdbTmp], axis=0)
    ...
    for (y,x) in zip(a1y,a1x):  # in idx_db loop
        ...
        if elevPath !='':
            ...
            a1elevflag = np.array([True])
       
        #--- Storm top ---
        if (stopPath !='')and(type_stop=='rng'):
            stormtop = a2stop[y,x]
            a1stopflag = ma.masked_inside( a1stopdb - stormtop, -MAX_STOP_DIFF, MAX_STOP_DIFF ).mask
        else:
            a1stopflag = np.array([True])

 
        #-- Screen DB candidates --
        a1flagNS   = a1prflagNS * a1t2mflag * a1tqvflag * a1elevflag * a1revflag * a1stopflag
        a1flagMS   = a1prflagMS * a1t2mflag * a1tqvflag * a1elevflag * a1revflag * a1stopflag
        ...
        if (a1flagNS.sum()<DB_USE_MINREC):
            print 'the Number of records are too small (NS)'
            print '# of records=',a1flagNS.sum()
            print 'Skip' 
            continue
        if (a1flagMS.sum()<DB_USE_MINREC):
            print 'the Number of records are too small (MS)'
            print '# of records=',a1flagMS.sum()
            print 'Skip' 
            continue
        #------------------------------
        ...
        #a2prwatprofNSSC= a2prwatprofNS[a1flagNS]  # stop
        ...
        if stopPath != '':   # stop
            a1stopdbMSSC = a1stopdb[a1flagMS]
            a1stopdbNSSC = a1stopdb[a1flagNS]


        #-- Storm top ---
        if (stopPath !='')and(type_stop=='wgt'):
            stormtop = a2stop[y,x]
            a1epc    = np.concatenate([a1epc, np.array([stormtop])])
            a2epcdbMSSC = np.concatenate([a2epcdbMSSC, a1stopdbMSSC.reshape(-1,1)],axis=1)
            a2epcdbNSSC = np.concatenate([a2epcdbNSSC, a1stopdbNSSC.reshape(-1,1)],axis=1)
        ...
        if flag_top_var ==1:
            ...
            #a3top_prwatprofNS[y,x,:] = db.get_var('precip_water_prof_NS', nrec=1, origin=topirecNS).flatten()[-NLEV_PRECIP:]   # stop
        ...
        #prwatprofNS   = (ma.masked_less(a2prwatprofNSSC[a1boolwtNS],0) * a1wtNS.reshape(-1,1)).sum(axis=0) / wtsumNS  # stop
        #a3prwatprofNS[y,x,:]    = prwatprofNS.filled(-9999.)   # stop
...
#np.save(outDir + '/prwatprofNS.%s.npy'%(stamp), a3prwatprofNS)  # stop
...
if flag_top_var ==1:
    ...
    #np.save(outDir + '/top-prwatprofNS.%s.npy'%(stamp), a3top_prwatprofNS) # stop
