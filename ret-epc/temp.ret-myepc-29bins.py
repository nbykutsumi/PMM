...
for i,idx_db in enumerate(lidxset):
    ...
    if MAX_TB_RMSD>0:
        a2tbdb  = ma.masked_invalid(a2tbdb)

    a2prwatprofNS = ma.masked_invalid(a2prwatprofNS)

    a1t2mdb = ma.masked_invalid(a1t2mdb)
    a1revdb = ma.masked_invalid(a1revdb)
    if tqvPath !='':
        a1tqvdb = ma.masked_invalid(a1tqvdb)
    if elevPath !='':
        a1elevdb= ma.masked_invalid(a1elevdb)
    ...
    for (y,x) in zip(a1y,a1x):  # in idx_db loop
        ...
        a1wtMS[idxtopMS] = 0.607  # np.expr(-0.5) = 0.606530..
        a1wtNS[idxtopNS] = 0.607  # np.expr(-0.5) = 0.606530..
