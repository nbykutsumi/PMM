...
void tc_ssmis(float *Tc_S1_buf, float *Tc_S2_buf, float *Tc_S3_buf, float *Tc_S4_buf,
          int nl_S1, int ns_S1, int nc_S1, 
          int nl_S2, int ns_S2, int nc_S2, 
          int nl_S3, int ns_S3, int nc_S3, 
          int nl_S4, int ns_S4, int nc_S4, 
          float *lat_S1_buf, float *lon_S1_buf, float *lat_S4_buf, float *lon_S4_buf,
          ...
          float *tc)
{
  ...
  for (i=0; i< nl_S1; i++) {
    for (j=0; j<ns_S1; j++) {
      ...
      lat2= lat_S4_buf[idx2];
      lon2= lon_S4_buf[idx2];
      ...
      qual= qual_S4_buf[idx2];
      printf("%4d ", qual);
      if ( qual >= 0 ) {
        for (k= 0; k< nc_S4; k++) {
          idx1= i*ns_S4*nc_S4 + (2*j+1)*nc_S4 + k;
          tb= Tc_S4_buf[idx1];
          if ( tb > 20 && tb < 400 ) {
            if ( qual > 0 ) tb*= -1.0;
      ...
      qual= qual_S3_buf[idx2];
      printf("%4d ", qual);
      if ( qual >= 0 ) {
        for (k= 0; k< nc_S3; k++) {
          idx1= i*ns_S3*nc_S3 + (2*j+1)*nc_S3 + k;
          tb= Tc_S3_buf[idx1];
          if ( tb > 20 && tb < 400 ) {
            if ( qual > 0 ) tb*= -1.0;
            tb2[k+nc_S1+nc_S2+nc_S4]= tb;
          }
        }
      }
