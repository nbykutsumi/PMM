...
#lrettype = ['GPROF'] 
#lrettype = ['NScmb']
lcmbrange = [[0.1, 9999],[0.1,1],[1,10],[10,9999]]
            ...
            for surftype in lsurftype:
                ...
                for Year,Mon in lYM:
                    ...
                    for orbit in lorbit:
                        ...
                        print obstype,rettype,surftype,Year,Mon,Day ,oid
                        #-- Read surface type ----
                ...
                if calcflag is True:
                    ...
                    for cmbrange in lcmbrange:
                        cmbmin,cmbmax = cmbrange
                        #a1flag = ma.masked_greater(a1obs, cmbmin).mask
                        a1flag = ma.masked_inside(a1obs, cmbmin, cmbmax).mask
                        ...
                        ddat[cmbmin,cmbmax,'num']  = num
                        ddat[cmbmin,cmbmax,'cc']   = cc
                        ddat[cmbmin,cmbmax,'rmse'] = rmse
                        ddat[cmbmin,cmbmax,'nme']  = nme
                ...
                picklepath = pickleDir + '/dict.cmbrange.%s.bfile'%(stamp)
                if calcflag == True:
...
for season in lseason:
    lout = []
    lsrange=['%.1f-%.1f'%(cmbmin,cmbmax) for (cmbmin,cmbmax) in lcmbrange]
    llabel= ['',   '',    '']+['rmse']*len(lsrange)+['nme']*len(lsrange) + ['cc']*len(lsrange)
    lout.append(llabel)

    llabel = ['Ref','Surf','PMW']+lsrange*3
    lout.append(llabel)
    ...
    csvpath = figDir + '/metrics.prec.cmbrange.%s.csv'%(season)
