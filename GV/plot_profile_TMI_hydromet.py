...
da1esurf = {i:deque([]) for i in lspecies}
...
for domain in ldomain:
    for YM in lYM:
        ...
        for species in lspecies:
            ...
            da1esurf[species].extend(aesurfTmp)


for species in lspecies:
    da2prof[species] = concatenate(da2prof[species], axis=0)
    da1esurf[species]= array(da1esurf[species])
...
for prtype in lprtype:
    for species in lspecies:
        ...
        amskP = ma.masked_outside(da1esurf[species], thmin, thmax).mask
    ...
    figPath = figDir + '/plt.prof-hydromet.%s.%.1fkm.minNum.%d.%s.png'%(prdName, thdist,minNum,prtype)
    plt.savefig(figPath)
    print figPath
    plt.clf()
