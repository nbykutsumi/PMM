...
for prtype in lprtype:
    ...
    ax.plot(dmean['conv'],a1y,'-',color='r', label='conv')
    ax.plot(dstd['conv'], a1y,'--',color='r')
    ax.plot(desurf['conv'], 0.1, 'v', color='r')
    
    ax.plot(dmean['strat'],a1y,'-',color='b', label='strat')    
    ax.plot(dstd['strat'],a1y,'--',color='b')    
    ...
    figPath = figDir + '/plt.aveprof.%s.%.1fkm.minNum.%d.%s.png'%(prdName, thdist,minNum,'prcp')
    plt.savefig(figPath)
    print figPath
    plt.clf()
