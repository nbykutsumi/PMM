...
for clstype in lclstype:
    ...
    for dattype in ldattype:
        ...
        # title
        stitle  = '%s %s %.1fkm minNum=%d'%(dattype, clstype, thdist, minNum)
        stitle  = stitle + '\n' + '%04d.%02d-%04d.%02d'%(iYM[0],iYM[1],eYM[0],eYM[1])
        plt.title(stitle)
        figDir  = '/work/a01/utsumi/GPMGV/fig'
        figPath = figDir + '/plot.dt-lev.%s.runmean.%s.%.1fkm.minNum.%d.%s.%s.png'%(cls, prdName, thdist, minNum, dattype, clstype)
        plt.savefig(figPath)
        print figPath
        plt.close()
